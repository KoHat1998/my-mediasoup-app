...
async function loadDevice(){ if (device) return; const caps = await new Promise(res => socket.emit('getRtpCapabilities', res)); device = new mediasoupClient.Device(); await device.load({ routerRtpCapabilities: caps }); }
...
async function produceOrReplace(kind, track){ if (kind==='video'){ if (videoProducer) await videoProducer.replaceTrack({ track }); else videoProducer = await sendTransport.produce({ track }); }
  else { if (!track) return; if (audioProducer) await audioProducer.replaceTrack({ track }); else audioProducer = await sendTransport.produce({ track }); } }
...
async function goLiveWithCamera(){ el('status').textContent = 'Requesting camera/mic…'; skeleton(true);
  const stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true }).catch(e => { el('status').textContent = 'Failed to get camera/mic: ' + e.message; throw e; });
...
async function shareScreen(){ el('status').textContent = 'Requesting screen…'; skeleton(true);
  const stream = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true }).catch(e => { el('status').textContent = 'Failed to share screen: ' + e.message; throw e; });
  const screenVideo = stream.getVideoTracks()[0]; if (screenVideo) screenVideo.onended = async () => { toast('Screen share ended'); try { await goLiveWithCamera(); } catch {} };
...
// viewer count scoped to your room
...
// join your channel room so you also see viewer count
