...
async function getFetch() { if (typeof fetch === 'function') return fetch; const { default: nodeFetch } = await import('node-fetch'); return nodeFetch; }
async function httpForm(url, fields = {}, headers = {}) {
  ...
  if (!r.ok) { const msg = (data && (data.message || data.error)) || text || `HTTP ${r.status}`; throw new Error(`HTTP ${r.status} – ${msg}`); }
  return data ?? {};
}
...
const io = new Server(server, {
  cors: { origin: ['http://localhost:3000', 'http://127.0.0.1:3000', 'https://livenix.htetaungthant.com'], methods: ['GET','POST','PATCH','OPTIONS'], allowedHeaders: ['Content-Type','Authorization'] }
});
...
const lives = new Map();
function makeSlug(title = 'live') { const base = String(title).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '').slice(0, 40) || 'live'; return `${base}-${Math.random().toString(36).slice(2, 6)}`; }
...
function getOrCreateLiveFromHint(hint) {
  ...
  const id = hint; const live = { id, slug: hint, title: `Live ${hint}`, description: null, hostUserId: 'auto', isLive: true, createdAt: new Date().toISOString(), endedAt: null, state: null };
...
// REST (unchanged as before) — omitted for brevity in this snippet
app.post('/api/lives', requireAuthHeader, async (req, res) => {
  ...
  const live = { id, slug, title: title || 'Untitled Live', description: description || null, hostUserId: 'token:' + (req.user.token || '').slice(0, 12), host: { name: (hostName || '').toString().slice(0,80) || null, email: (hostEmail || '').toString().slice(0,120) || null }, isLive: true, createdAt: new Date().toISOString(), endedAt: null, state: null };
  lives.set(id, live); ensureLiveState(id);
  try { await httpForm(`${AUTH_BASE}/streams/start`, { stream_id: live.id, title: live.title }, { Authorization: req.user?.raw || '' }); io.emit('streams:changed', { type: 'start', id: live.id, slug: live.slug, title: live.title }); } catch (e) { console.warn('⚠️ Failed to sync /streams/start:', e.message); }
  res.json(live);
});
app.get('/api/lives', requireAuthHeader, async (_req, res) => {
  const list = Array.from(lives.values()).filter(l => l.isLive).map(l => ({ id:l.id, slug:l.slug, title:l.title, description:l.description, createdAt:l.createdAt, hostName:l.host?.name || null, hostEmail:l.host?.email || null }));
...
app.patch('/api/lives/:id/end', requireAuthHeader, async (req, res) => {
  ...
  if (live.state) { try { for (const [, t] of live.state.transports) { try { t.send?.close(); } catch {} try { t.recv?.close(); } catch {} } try { live.state.videoProducer?.close(); } catch {} try { live.state.audioProducer?.close(); } catch {} } catch {} }
  try { await httpForm(`${AUTH_BASE}/streams/stop`, { stream_id: live.id }, { Authorization: req.user?.raw || '' }); io.emit('streams:changed', { type: 'stop', id: live.id, slug: live.slug }); } catch (e) { console.warn('⚠️ Failed to sync /streams/stop:', e.message); }
  res.json({ ok: true });
});
...
app.use((req, res, next) => { if (req.path.endsWith('.html')) { res.set('Cache-Control','no-store, no-cache, must-revalidate, proxy-revalidate'); res.set('Pragma','no-cache'); res.set('Expires','0'); res.set('Surrogate-Control','no-store'); } next(); });
...
io.engine.on('connection_error', (err) => {
  console.error('engine.io connection_error:', { code: err.code, message: err.message, context: err.context });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('produce', async ({ kind, rtpParameters }, cb) => {
    try {
      ...
      if (kind === 'video') { await state.videoProducer?.close().catch(()=>{}); state.videoProducer = producer; io.to(roomName).emit('newProducer', { kind:'video', slug: live.slug }); }
      else { await state.audioProducer?.close().catch(()=>{}); state.audioProducer = producer; io.to(roomName).emit('newProducer', { kind:'audio', slug: live.slug }); }
  ...
  socket.on('consume', async ({ rtpCapabilities, kind }, cb) => {
    try {
      ...
      if (!prod) { console.warn(`[consume] no ${kind} producer for live ${live.slug}`); return errAck(cb, `no ${kind} producer`); }
      const can = router.canConsume({ producerId: prod.id, rtpCapabilities });
      if (!can) {
        console.warn(`[consume] cannot consume ${kind} for live ${live.slug}`, {
          offered: (rtpCapabilities?.codecs || []).map(c => c.mimeType),
          router: (router.rtpCapabilities?.codecs || []).map(c => c.mimeType)
        });
        return errAck(cb, 'cannot consume');
      }
      const t = (state.transports.get(socket.id) || {}).recv;
      if (!t) return errAck(cb, 'no recv transport');
